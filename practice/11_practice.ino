...
float duty_neu = 2410;
  if(dist_raw<=dist_min) myservo.writeMicroseconds(_DUTY_MIN);
  else if(dist_raw>=dist_max) myservo.writeMicroseconds(_DUTY_MAX);
  else {
    duty_neu -= 1860*(360-dist_raw)/180;
    myservo.writeMicroseconds(duty_neu);
  }

  if(dist_max>=dist_raw && dist_raw>=dist_min) analogWrite(PIN_LED, 0);
  else analogWrite(PIN_LED, 255);
